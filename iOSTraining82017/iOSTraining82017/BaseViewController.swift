...
class BaseViewController: UIViewController, UIGestureRecognizerDelegate {
    ...
    func addBtnBackNav(tinColor: UIColor? = nil) {
        let backButton = self.createBtnNavWithImage(image: #imageLiteral(resourceName: "icon-back_filled"), target: self, action: #selector(BaseViewController.actionTouchBtnBack), tinColor: tinColor)
    ...
    private func createBtnNavWithImage(image: UIImage?, target: AnyObject?, action: Selector, tinColor: UIColor? = nil) -> UIButton {
        ...
        navButton.tintColor = tinColor ?? UIColor.white
        return navButton
    }
