...
enum TimelineRouter: URLRequestConvertible {
    ...
    func asURLRequest() throws -> URLRequest {
        ...
        switch self {
        case .getTimeline():
            guard let data = UserDefaults.standard.data(forKey: UserDefaultKey.tokenValue) else {break}
            ...
            urlRequest.addValue(HeaderValue.ApplicationJson, forHTTPHeaderField: HeaderKey.ContentType)
            urlRequest.addValue(token, forHTTPHeaderField: HeaderKey.Authorization)
        }
        return urlRequest
    }
}
